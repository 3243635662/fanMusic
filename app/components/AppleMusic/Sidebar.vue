    ...
    <div class="flex flex-col flex-1 min-h-0 gap-1">
      ...
      <div class="flex flex-col min-h-0 shrink-0">
        <div @click="isPlaylistOpen = !isPlaylistOpen"
          ...
          :class="[
            isFold ? 'justify-center px-0' : 'justify-between',
        ...
        <Transition name="fade-slide">
          <div v-show="isPlaylistOpen && !isFold"
      ...
      <div class="flex flex-col flex-1 min-h-0 pt-1">
        ...
        <!-- Queue List -->
        <Transition name="fade-slide">
          <div v-show="isQueueOpen && !isFold" class="flex-1 overflow-y-auto custom-scrollbar px-2 mt-1 space-y-0.5">
            <div v-for="track in musicStore.playQueue" :key="track.hash"
              ...
              <div class="w-8 h-8 rounded-lg overflow-hidden bg-white/5 shrink-0 relative border border-white/5">
                <img :src="processCover(track.cover || '')" @error="($event.target as HTMLImageElement).src = processCover('')"
                  class="w-full h-full object-cover" alt="Track Cover" />
                <div v-if="musicStore.currentTrack?.hash === track.hash" class="absolute inset-0 bg-primary/20 flex items-center justify-center">
                   <div class="w-1.5 h-1.5 rounded-full bg-primary animate-pulse shadow-[0_0_8px_var(--ui-primary)]"></div>
                </div>
              </div>
              ...
              <div class="flex-1 min-w-0">
                ...
                <div class="text-[10px] text-zinc-500 font-medium truncate uppercase tracking-tighter">{{ track.artist }}</div>
              </div>
            </div>
...
const processCover = (url: string) => {
  if (!url) return "http://imge.kugou.com/stdmusic/150/20210817/20210817080716564241.jpg";
