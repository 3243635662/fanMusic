<template>
  <div class="pb-24 scrollbar-hide">
    <AppleMusicPageHeader title="我的音乐" description="自建歌单与收藏">
      <template #extra>
        <LayoutsTipsButton @click="fetchPlaylists" text="刷新歌单"
          :icon="useIcon('refresh')" icon-class="w-4 h-4 text-primary"
    ...
    <div v-else-if="musicStore.playList.length" class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3 md:gap-6">
